...
export default{
  created () {
    ...
    localStorage.getItem('goods') && this.$store.replaceState({...this.$store.state, ...JSON.parse(localStorage.getItem('goods'))})
    ...
    window.addEventListener('beforeunload', () => {
      localStorage.setItem('goods', JSON.stringify({cartFoods:this.$store.state.cartFoods,shopGoods:this.$store.state.shopGoods}))
    })
  },
  beforeDestroy () {
