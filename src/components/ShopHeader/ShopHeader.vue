<template>
  <div class="shop-header" >
    <nav class="shop-nav" :style="{background :`url(${shopInfo.bgImg})`} ">
      <a class="back" @click="$router.back()">
        <i class="iconfont icon-arrow_left"></i>
      </a>
    </nav>
    <div class="shop-content" @click.stop="toggleShopShow">
    ...
    <!--因为数据是异步获取的，在显示时容易获取不到而导致报错，所以需要一个判断，一般是三层的属性容易出现这类问题-->
    <div class="shop-header-discounts" v-if="shopInfo.supports" @click.stop="toggleSupportShow">
    ...
    <transition name="fade">
      <div class="activity-sheet" v-show="supportShow">
        ...
        <!--
        这里有一层遮罩，层级比显示的内容低，而且占据整片屏幕，
        所以在这里设置就可完成之前的点击其他区域也关闭的效果
        昨天写的没成功也找到了原因，虽然在父元素中调用且停止了事件冒泡，但是因为没有发现有这层遮罩，所以将遮罩的冒泡也停止，所以不会触发
        -->
        <!--所以这里真的超简单，只要加一层遮罩就可以了！！！-->
